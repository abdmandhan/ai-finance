      ...
      try {
        ...
        if (state.requestedStartIso) {
          ...
          if (isPhysical(state.location)) {
            const prev = events
              .filter(
                (e) =>
                  Date.parse(e.end) <= reqStartMs && isPhysical(e.location),
              )
            ...
            if (prev) {
              ...
              const needMs = Math.max(
                bufferMs,
                tt ? tt.durationMinutes * 60_000 : 0,
              );
              if (reqStartMs - Date.parse(prev.end) < needMs)
                travelShortfall = true;
            }
          }
          ...
          return {
            selectedSlot: { start: reqStart, end: reqEnd },
        ...
        if (slots.length === 0) {
          return {
            result: {
              status: "failed" as const,
              summary: "No free slots were found in the next two weeks.",
            },
            _nextNode: NODES.finalize,
          };
        }
