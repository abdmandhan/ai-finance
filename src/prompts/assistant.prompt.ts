...
function systemPrompt(ctx: { nowIso: string; timezone: string }): string {
  return [
    ...
    "  who owes them, what's unpaid/overdue/draft/paid/voided, or totals by",
    "  supplier/category/period → call `financial_report` immediately with the question verbatim.",
    "  Do NOT ask which system (Xero IS their accounting system) and do NOT pre-clarify the period",
    "  or what counts as \"spend\" —",
