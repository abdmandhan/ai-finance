...
import { detectConflicts, findFreeSlots, isPhysical, type SchedulingPrefs } from './scheduling';
...
const D = (h: number, m = 0) => `2026-07-13T${String(h).padStart(2, '0')}:${String(m).padStart(2, '0')}:00.000Z`;
...
describe('findFreeSlots', () => {
  it('finds a slot in the gap, respecting buffer', () => {
    const slots = findFreeSlots([{ start: D(9), end: D(10) }], D(9), D(12), 30, prefs, 3);
    // First slot starts at/after 10:00 + 15m buffer.
    expect(Date.parse(slots[0].start)).toBeGreaterThanOrEqual(Date.parse(D(10, 15)));
  });
  ...
  it('pads the busy tail by travel time when larger than buffer', () => {
    ...
    expect(Date.parse(slots[0].start)).toBeGreaterThanOrEqual(Date.parse(D(11)));
  });
