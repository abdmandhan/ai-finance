...
export interface IAuditService {
  runStarted(input: { threadId: string; workflow: string; userId?: string }): void;
  toolCalled(input: { threadId: string; tool: string; ok: boolean; elapsedMs: number }): void;
  runFinished(input: { threadId: string; status: string; durationMs: number }): void;
}
